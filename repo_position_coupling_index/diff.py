...
from repo_position_coupling_index.model import CouplingIndex, CouplingRow, Flag
...
def render_diff(previous: CouplingIndex, current: CouplingIndex) -> str:
    ...
    added = sorted(
        set(current_couplings) - set(previous_couplings),
        key=lambda key: (key.repo_slug, key.pillar_id),
    )
    removed = sorted(
        set(previous_couplings) - set(current_couplings),
        key=lambda key: (key.repo_slug, key.pillar_id),
    )
    shared = sorted(
        set(previous_couplings) & set(current_couplings),
